/* ========================================================================== */
/*		Exp15_1.c : RS-232C Communication by Polling	              */
/* ========================================================================== */
...
TXD_char(unsigned char character)		/* transmit a character */
{ while(!TI);					// transmit ready ?
  TI = 0;					// if yes, TI=0
  SBUF = character;				//      and transmit a character
}

TXD_string(unsigned char *string)		/* transmit a string */
{ while(*string != '\0')
    { TXD_char(*string);
      string++;
    }
...
main()
{
  ...
  while(1)
    { switch(Key_input())			// key input
        { case 0x1C : DIG_SELECT = 0x01;
                      TXD_string("KEY1 was pressed.");
                      TXD_char(0x0D); TXD_char(0x0A);
                      break;
          case 0x1A : DIG_SELECT = 0x02;
                      TXD_string("KEY2 was pressed.");
                      TXD_char(0x0D); TXD_char(0x0A);
                      break;
          case 0x16 : DIG_SELECT = 0x04;
                      TXD_string("KEY3 was pressed.");
                      TXD_char(0x0D); TXD_char(0x0A);
                      break;
          case 0x0E : DIG_SELECT = 0x08;
