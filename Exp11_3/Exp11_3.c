...
LCD_2hex(unsigned char number)			/* display 2-digit hex number */
{ unsigned char i;
...
LCD_3d(unsigned int number)			/* display 3-digit decimal number */
...
LED_2hex(unsigned char number)			/* display 2-digit hex number */
{
...
LED_3d(unsigned int number)			/* display 3-digit decimal number */
{ unsigned char i,flag;

  flag = 0;
  i = number/100;                               // 10^2
  if(i == 0) pattern[5] = 0x00;
  else {     pattern[5] = Bin2LED(i);
             flag = 1;
       }
