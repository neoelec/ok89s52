...
LCD_3d(unsigned char number)			/* unsigned 3-digit decimal number */
...
LCD_s3d(signed char number)			/* signed 3-digit decimal number */
...
Ucompare(unsigned char a, unsigned char b)	/* unsigned char compare */
{ LCD_command(0xC3);				// display first data
  ...
  if(a == b)      LCD_data('=');
  else if (a > b) LCD_data('>');
  else            LCD_data('<');
  Delay_ms(2000);
}

Compare(signed char a, signed char b)		/* signed char compare */
