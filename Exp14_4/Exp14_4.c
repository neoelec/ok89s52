...
LCD_2BCD(unsigned char number)			/* display 2-digit BCD number */
{ unsigned char i;
...
DSP_RTC()					/* display clock from RTC */
...
unsigned char BCD_increment(unsigned char number) /* BCD increment */
{ unsigned char i;
...
unsigned char BCD_decrement(unsigned char number) /* BCD decrement */
{ unsigned char i;
...
Cursor_right()					/* go cursor right */
{
  if(cursor == 0xCF)      cursor = 0x84;
  else if(cursor == 0x84) cursor = 0x87;
  else if(cursor == 0x87) cursor = 0x8A;
  else if(cursor == 0x8A) cursor = 0x8E;
...
{ unsigned char year, month, date, weekday, hour, minute, second;
  ...
  switch(cursor)
    { case 0x84 : year = RTC_YEAR;              // in case of year
                  if(year == 0x99)      year = 0x00;
                  else                  year = BCD_increment(year);
                  RTC_YEAR = year;
                  break;
      case 0x87 : month = RTC_MONTH;            // in case of month
                  if(month == 0x12)     month = 0x01;
                  else                  month = BCD_increment(month);
                  RTC_MONTH = month;
                  break;
      case 0x8A : date = RTC_DATE;              // in case of date
                  if(date == 0x31)      date = 0x01;
                  else                  date = BCD_increment(date);
                  RTC_DATE = date;
                  break;
      case 0x8E : weekday = RTC_WEEKDAY;        // in case of weekday
                  if(weekday == 0x07)   weekday = 0x01;
                  else                  weekday++;
                  RTC_WEEKDAY = weekday;
                  break;
      case 0xC3 : hour = RTC_HOUR;              // in case of hour
                  if(hour == 0x12)      hour = 0x01;
                  else if(hour == 0x92) hour = 0x81;
                  else                  hour = BCD_increment(hour);
                  RTC_HOUR = hour;
                  break;
      case 0xC6 : minute = RTC_MINUTE;          // in case of minute
                  if(minute == 0x59)    minute = 0x00;
                  else                  minute = BCD_increment(minute);
                  RTC_MINUTE = minute;
                  break;
      case 0xC9 : second = RTC_SECOND;          // in case of second
                  if(second == 0x59)    second = 0x00;
                  else                  second = BCD_increment(second);
                  RTC_SECOND = second;
                  break;
      case 0xCC : hour = RTC_HOUR;              // in case of AM/PM
                  if(hour < 0x80)       hour |= 0x80;
                  else                  hour &= 0x7F;
                  RTC_HOUR = hour;
                  break;
      default   : break;
  ...
  switch(cursor)
    { case 0x84 : year = RTC_YEAR;              // in case of year
      ...
      case 0x8E : weekday = RTC_WEEKDAY;        // in case of weekday
                  ...
                  break;
      case 0xC3 : hour = RTC_HOUR;              // in case of hour
                  if(hour == 0x01)      hour = 0x12;
                  else if(hour == 0x81) hour = 0x92;
  ...
  while(1)
    { DSP_RTC();                                // display clock on LCD module
      ...
      switch(Key_input())			// key input
        { case 0x0E : Cursor_left();		// if KEY4, go cursor left
                      break;
          case 0x16 : Cursor_right();		// if KEY3, go cursor right
                      break;
          case 0x1A : Increment();		// if KEY2, increment time
                      break;
          case 0x1C : Decrement();		// if KEY1, decrement time
                      break;
          default   : break;
        }
    }
