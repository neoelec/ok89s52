...
LCD_3d2(unsigned int number)			/* display time xxx.xx */
...
main()
{
  ...
  while(1)
    { switch(Key_input())
        { case 0x1C : if(run_flag == 1) break;	// if KEY1, start
                      ...
                      break;
          case 0x16 : if(run_flag == 1) break;	// if KEY3, reset
                      LCD_string(0xC0,"  000.00 [SEC]  ");
                      time = 0;			// clear time
                      DIG_SELECT = 0x04;	// LED3 on
                      break;
          case 0x0E : if(run_flag == 0) break;	// if KEY4, stop
                      ET0 = 0;
