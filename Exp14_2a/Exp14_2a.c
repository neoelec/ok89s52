...
LCD_2d(unsigned char number)			/* display time xx */
{
...
{ unsigned char colon;
  ...
  int_count--;
  if(int_count != 0) return;			// interrupt 20 times ?
  int_count = 20;
  colon_flag ^= 0x01;				// colon on or off ?
  if(colon_flag == 0) colon = ' ';
  else                colon = ':';
  LCD_command(0xC4);
  LCD_data(colon);
  LCD_command(0xC7);
...
{ 
  if(cursor == 0xC2)      cursor = 0xCF;
  else if(cursor == 0xCF) cursor -= 4;
  else                    cursor -= 3;
  LCD_command(cursor);
}
...
{ 
  if(cursor == 0xCF)      cursor = 0xC2;
  else if(cursor == 0xCB) cursor += 4;
  else                    cursor += 3;
  LCD_command(cursor);
}
...
{ switch(cursor)
    { case 0xC2 : hour++;			// in case of hour
                  if(hour == 13) hour = 1;
                  LCD_command(0xC2); LCD_2d(hour);
                  LCD_command(cursor);
                  break;
      case 0xC5 : minute++;			// in case of minute
                  if(minute == 60) minute = 0;
                  LCD_command(0xC5); LCD_2d(minute);
                  LCD_command(cursor);
                  break;
      case 0xC8 : second++;			// in case of second
                  if(second == 60) second = 0;
                  LCD_command(0xC8); LCD_2d(second);
                  LCD_command(cursor);
                  break;
      case 0xCB : if(ampm == 'A') ampm = 'P';	// in case of AM/PM
                  else            ampm = 'A';
...
{ switch(cursor)
    { case 0xC3 : hour--;			// in case of hour
                  if(hour == 0) hour = 12;
                  LCD_command(0xC2); LCD_2d(hour);
                  LCD_command(cursor);
                  break;
      case 0xC6 : minute--;			// in case of minute
                  if(minute == -1) minute = 59;
                  LCD_command(0xC5); LCD_2d(minute);
                  LCD_command(cursor);
                  break;
      case 0xC9 : second--;			// in case of second
