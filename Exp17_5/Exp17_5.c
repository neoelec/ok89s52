...
LCD_2hex(unsigned char number)			/* display HEX number xxH */
{ unsigned char i;
...
LCD_s2d1(unsigned char number)			/* display signed real number +xx.x */
{ unsigned char i;
  unsigned int j;
  float voltage;
...
{ unsigned char volt;
  ...
  while(1)
    { switch(Key_input())			// key input
        { case 0x1C : volt += 0x10;		// if KEY1, increment by 0x10
                      DAC_CS = volt;
                      break;
          case 0x1A : volt -= 0x10;		// if KEY2, decrement by 0x10
                      DAC_CS = volt;
                      break;
          case 0x16 : volt += 0x01;		// if KEY3, increment by 0x01
                      DAC_CS = volt;
                      break;
          case 0x0E : volt -= 0x01;		// if KEY4, decrement by 0x01
                      DAC_CS = volt;
                      break;
          default   : break;
        }
